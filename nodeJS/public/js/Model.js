class Calculator {
  ...
  lengthMultiAvg (array1, array2) {
    ...
    const product3 = lengthOfArray * this.avgOfArray(array1) * this.avgOfArray(array2)
    return product3
  }

  // This function means: sumOfArray1*sumOfArray2
  sumMultiSum (array1, array2) {
    const product2 = this.sumOfArray(array1) * this.sumOfArray(array2)
    return product2
  }

  // This function use to reduce the code
  subtraction1 (a, b) {
    const difference = this.lengthMultiSum(a, this.sumOfArray(this.products(a, b))) - this.sumOfArray(a) * this.sumOfArray(b)
    return difference
  }

  // This function use to reduce the code
  subtraction2 (a, b) {
    const difference2 = this.sumOfArray(this.products(a, b)) - this.lengthMultiAvg(a, b)
    return difference2
  }
  ...
  results (a, b) {
    // Part of Correlation
    const s = this.subtraction1(a, b) / Math.sqrt(this.subtraction1(a, a) * this.subtraction1(b, b))
    this.Rxy = s
    this.R2 = Math.pow(s, 2)

    // Part of Regression
    const h = this.subtraction2(a, b) / this.subtraction2(a, a)
    this.Bata1 = h
